...
library(reticulate)
np <- import("numpy")
check_imputer = np$load('/Users/yanyul/Desktop/tmp/haplotype-po/test_sanity_imputer_chr19.npy')
dim(check_imputer)
...
snp_map_19 = snp_map %>% filter(chrom == 19)

gwas_father_extracted = left_join(snp_map_19, gwas_father %>% filter(CHR == 19), by = c('pos' = 'BP'))
