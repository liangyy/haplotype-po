...
```{r stat, fig.width=5, fig.height=5}
df_re %>% filter(type == 'beta = 1') %>% 
  ggplot() + geom_boxplot(aes(x = z, group = z, y = stat)) + facet_grid(z_type~method) +
...
<!-- ```{r} -->
<!-- plot(df_re %>% filter(type == 'case1') %>% pull(stat), df_re %>% filter(type == 'case3', z == 0.5) %>% pull(stat)) -->
<!-- ``` -->
