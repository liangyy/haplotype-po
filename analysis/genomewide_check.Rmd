...
gwas_ht_bonferroni = gwas_self %>% filter(pval < 0.01 / n(), !low_confidence_variant)
...
<!-- ```{r adqq} -->
<!-- df_ad = df %>% filter(trait == 'AD') %>% mutate(pval = beta2pval(beta, se)) %>% group_by(impute) %>% mutate(pexp = rank(pval) / (n() + 1)) %>% ungroup() -->
<!-- df_ad %>% ggplot() + geom_point(aes(x = -log(pexp), y = -log(pval), color = impute)) -->
<!-- ``` -->
