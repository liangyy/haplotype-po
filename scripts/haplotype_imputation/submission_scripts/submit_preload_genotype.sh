...
outprefix=/lambda_stor/data/yanyul/UKB/haplotype_imputation/haplotype_impute_otf_multi_chr/preload


for i in `seq $1 $2`
do
  screen -dmS preload-$i bash -c "bash run.sh $genodir/ukb_hap_v2_to_hdf5.chr$i.h5 $snpdir/chr$i $inprefix $outprefix.chr$i"
done
